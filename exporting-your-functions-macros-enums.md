...
A version may be specified optionally, but just the name itself is sufficient enough. Additionally, `#export` does not need to be per script. You can have a single script that contains all of your exports, as it's specifically targeting global keywords. 
```gml
// Script A
...
And as an additional note to assets, the above namespace scope reference can be used for public assets too, if there's two assets with the same name, one in the prefab, one in the project. 
